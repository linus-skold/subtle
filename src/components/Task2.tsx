...
import { parseEstimate, parseProgress } from '@/utils/time.utils';


const Task2 = ({ order, task }: { order: number; task: Task }) => {
  ...
  const { task_name: title, estimate, progress } = task;
  ...
  return (
    ...
    >
      <div
        className={`bg-gray-800 m-4 p-4 rounded-lg relative transition-all duration-${completeTime} ease-in-out ${
      ...
      >
        ...
        <div className={`flex justify-between transition-opacity duration-200`}>
          <p className="text-gray-400 text-sm">{parseEstimate(estimate)}</p>
          <p className="text-gray-400 text-sm">{parseProgress(progress)}</p>
        </div>
        <div
